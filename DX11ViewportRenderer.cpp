...
:	MViewportRenderer("DX11ViewportRenderer")
{
	// Set the ui name
	fUIName.set( "Bankau VR Renderer");
	...
	if (VR_IsHmdPresent()) {
		m_hmd = VR_Init(&eError, VRApplication_Scene);

		if (eError != vr::VRInitError_None) {
			MGlobal::displayError("OpenVR Initialization Error: " + (MString)VR_GetVRInitErrorAsEnglishDescription(eError));
		}
	}
	else {
		MGlobal::displayError("OpenVR: No HMD present.");
	}
#endif
}
...
/* virtual */ 
MStatus	
DX11ViewportRenderer::render(const MRenderingInfo &renderInfo)
{
	MStatus status;
...
/* virtual */ 
bool	
DX11ViewportRenderer::nativelySupports( MViewportRenderer::RenderingAPI api, 
									   float version )
{
...
/* virtual */ bool	
DX11ViewportRenderer::override( MViewportRenderer::RenderingOverride override )
{
	// Check override
...
bool					
DX11ViewportRenderer::translateCamera( const MRenderingInfo &renderInfo )
//
// Description:
